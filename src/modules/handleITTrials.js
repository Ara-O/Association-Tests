...
function nextFaceToMemorize(thisval) {
    ...
    if (thisval.currentlyVisible < thisval.facedataclone.length - 1) {
        thisval.facedataclone[thisval.currentlyVisible].visibility = "none";
        thisval.currentlyVisible++;
        thisval.facedataclone[thisval.currentlyVisible].visibility = "block";
    } else {
        // End of one cycle
...
function proceedToNextSection(thisval, IT_Trials, section) {
    if (thisval.inaccurateTrial === false) {
        if (thisval.currentProgression < 5) {
            ...
            thisval.facedataclone = JSON.parse(JSON.stringify(thisval.facedata));
            thisval.facedatashuffled = JSON.parse(JSON.stringify(thisval.facedata));
            thisval.facedatashuffled = thisval.shuffleObjects(thisval.facedatashuffled);
...
function validateChoice(choice, thisval) {
    ...
    thisval.currentFace = thisval.facedata[thisval.currentlyVisible].cN;
    ...
    if (thisval.currentlyVisible !== thisval.facedata.length) {
        ...
        if (choice == thisval.currentFace) {
            ...
            document.querySelector(".correctchoice-img").src = `${thisval.getImg(
                thisval.currentFace
            )}`;
            document.querySelector(".choices").style.display = "none";
        }
